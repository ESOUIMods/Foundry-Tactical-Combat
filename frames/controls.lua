...
function FTC.Frames:Controls()
  ...
  player.label = FTC.UI:Label("FTC_PlayerFrame_Label", player, "inherit", { CENTER, CENTER, 0, 0 }, FTC.UI:Font("Trajan Pro", 24, true), nil, { 1, 1 }, GetString(FTC_PF_Label), true)
  ...
  local target = FTC.UI:Control("FTC_TargetFrame", FTC_UI, { FTC.Vars.TargetFrameWidth, FTC.Vars.TargetFrameHeight }, FTC.Vars.FTC_TargetFrame, true)
  target.backdrop = FTC.UI:Backdrop("FTC_TargetFrame_BG", target, "inherit", { CENTER, CENTER, 0, 0 }, { 0, 0, 0, 0.4 }, { 0, 0, 0, 1 }, nil, true)
  target.label = FTC.UI:Label("FTC_TargetFrame_Label", target, "inherit", { CENTER, CENTER, 0, 0 }, FTC.UI:Font("Trajan Pro", 24, true), nil, { 1, 1 }, GetString(FTC_TF_Label), true)
  target.backdrop:SetEdgeTexture("", 16, 4, 4)
  target:SetDrawTier(DT_HIGH)
  target:SetAlpha(FTC.Vars.FrameTargetOpacityOut / 100)
  ...
  lplate:SetHidden(not FTC.Vars.TargetFrameTitle)
  ...
  local parent = FTC.TARGET_WINDOW
  ...
  group.label = FTC.UI:Label("FTC_GroupFrame_Label", group, "inherit", { CENTER, CENTER, 0, 0 }, FTC.UI:Font("Trajan Pro", 24, true), nil, { 1, 1 }, GetString(FTC_GF_Label), true)
  ...
  for i = 1, STANDARD_GROUP_SIZE_THRESHOLD do
    ...
    ultimate.bar:SetDrawLayer(DL_OVERLAY)

    -- Set health labels on top of everything
    health.pct:SetDrawTier(DT_HIGH)
    health.current:SetDrawTier(DT_HIGH)
    health.pct:SetDrawLevel(3)
    health.current:SetDrawLevel(3)
    health.pct:SetDrawLayer(DL_OVERLAY)
    health.current:SetDrawLayer(DL_OVERLAY)
  ...
  raid.label = FTC.UI:Label("FTC_RaidFrame_Label", raid, "inherit", { CENTER, CENTER, 0, 0 }, FTC.UI:Font("Trajan Pro", 24, true), nil, { 1, 1 }, GetString(FTC_RF_Label), true)
  ...
  -- Callback
  CALLBACK_MANAGER:FireCallbacks("FTCFrameControlsUpdate")
end
...
function FTC.Frames:Fade(unitTag, frame)
  -- Handle target frame separately
  local opacityOut = (frame == FTC_TargetFrame) and FTC.Vars.FrameTargetOpacityOut or FTC.Vars.FrameOpacityOut
  local opacityIn = (frame == FTC_TargetFrame) and FTC.Vars.FrameTargetOpacityIn or FTC.Vars.FrameOpacityIn
  ...
  if (frame.fadeIn == nil or FTC.Frames.resetAnim) then
    ...
    animIn:SetAlphaValues(opacityOut / 100, opacityIn / 100)
  ...
  if (frame.fadeOut == nil or FTC.Frames.resetAnim) then
    ...
    animOut:SetAlphaValues(opacityIn / 100, opacityOut / 100)
  ...
  if (not displayFrame and (zo_round(frame:GetAlpha() * 100) == opacityOut) or displayFrame and (zo_round(frame:GetAlpha() * 100) == opacityIn)) then return end
...
function FTC.Frames:Regen(unitTag, unitAttributeVisual, powerType, duration)
  ...
  elseif (powerType == COMBAT_MECHANIC_FLAGS_STAMINA) then attrType = "Stamina"
  elseif (powerType == COMBAT_MECHANIC_FLAGS_MAGICKA) then attrType = "Magicka"
  else return end
